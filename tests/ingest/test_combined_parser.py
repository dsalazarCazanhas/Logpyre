...
# Minimal combined line — no authenticated user, no referer
LINE_MINIMAL = (
    '93.184.216.34 - - [15/Mar/2024:10:22:01 +0000] '
    '"GET /index.html HTTP/1.1" 200 1024 "-" "Mozilla/5.0"'
)

# Line with authenticated user and a full referer
...
# POST request producing a 404
LINE_404 = (
    '192.168.1.50 - - [17/Mar/2024:12:00:00 +0000] '
    '"DELETE /resource/99 HTTP/1.1" 404 0 "-" "HTTPie/3.2.1"'
)

# TLS ClientHello probe — Nginx-escaped binary request field
LINE_TLS = (
...
# Unknown raw probe (no recognised prefix, not valid HTTP)
LINE_UNKNOWN_RAW = (
